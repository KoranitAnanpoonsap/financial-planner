...
export default function CFPGoalBaseCalculated() {
    ...
    return Goal && (
        <div className="bg-tfpa_blue text-center p-6 text-xl rounded-3xl space-y-6 text-white font-bold">
          ...
          <div className="text-lg grid grid-cols-2 text-start mx-8">
            <div>เงินรวมปัจจุบันในการลงทุน: {Number(Goal.totalInvestment).toLocaleString('en-us', {minimumFractionDigits: 2, maximumFractionDigits: 2,})} บาท</div>
            <div>กระเเสเงินสดสุทธิเฉลี่ยต่อปี: {Number(NetIncome).toLocaleString('en-us', {minimumFractionDigits: 2, maximumFractionDigits: 2,})} บาท</div>
            <div>ผลตอบเเทนต่อปีของพอร์ตที่ลงทุนในปัจจุบัน: {PortfolioReturn.toLocaleString()}%</div>
            <div>อัตราการเติบโตของกระเเสเงินสดสุทธิต่อปี: {(Goal.netIncomeGrowth*100).toFixed(2).toLocaleString()}%</div>
            <div>เงินรวมปัจจุบันในการลงทุนในอีก {Goal.goalPeriod.toLocaleString()} ปี: {fvOfCurrentInvestment.toLocaleString()} บาท</div>
          </div>
        </div>
    )
  }
  ...
  const CalculateGoalBasedGoalDetails = () => {
    return Goal && (
        <div className="bg-tfpa_blue text-center p-6 text-xl rounded-3xl space-y-6 text-white font-bold">
          <div>รายละเอียดเป้าหมาย
          </div>
          <div className="text-lg grid grid-cols-2 text-start mx-8">
            <div className="">จำนวนเงินเพื่อเป้าหมาย: {Number(Goal.goalValue).toLocaleString('en-us', {minimumFractionDigits: 2, maximumFractionDigits: 2,})} บาท</div>
            <div className="">ระยะเวลาเป้าหมาย: {Goal.goalPeriod.toLocaleString()} ปี</div>
          </div>
        </div>
    )
  }
          ...
          >
            <div className="flex flex-col gap-8 mx-32">
              ...
              <div className="text-tfpa_gold text-xl font-bold grid grid-cols-2 text-start mx-12">
                <span>จำนวนเงินที่ลูกค้าต้องเก็บออมต่อปี: {GoalAnnualSaving < 0
                    ? 0
                    : Number(GoalAnnualSaving).toLocaleString('en-us', {minimumFractionDigits: 2, maximumFractionDigits: 2,})
                } บาท</span>
                <span>เฉลี่ยเดือนละ: {GoalAnnualSaving < 0
                    ? 0
                    : Number(GoalAnnualSaving/12).toLocaleString('en-us', {minimumFractionDigits: 2, maximumFractionDigits: 2,})
                } บาท</span>
              </div>
